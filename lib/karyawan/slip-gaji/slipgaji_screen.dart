...
class _SlipGajiScreenState extends State<SlipGajiScreen> {
  ...
  String _mapStatus(String backendStatus) {
    switch (backendStatus.toLowerCase()) {
      ...
      case 'terlambat_masuk': return 'Terlambat Masuk';
      case 'pulang_duluan': return 'Pulang Duluan';
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      ...
      case 'Terlambat Masuk': return Colors.orange;
      case 'Pulang Duluan': return Colors.deepOrange;
      case 'Cuti': return Colors.blue;
      case 'Izin': return Colors.purple;
  ...
  IconData _getStatusIcon(String status) {
    switch (status) {
      ...
      case 'Terlambat Masuk': return Icons.access_time;
      case 'Pulang Duluan': return Icons.exit_to_app;
  ...
  String _getKeterangan(Map<String, dynamic> data) {
    switch (data['status']) {
      case 'Hadir': return 'Hadir tepat waktu';
      case 'Terlambat Masuk': return 'Terlambat masuk kerja';
      case 'Pulang Duluan': return 'Pulang lebih awal';
      case 'Cuti': return 'Cuti disetujui';
      case 'Izin': return 'Izin disetujui';
      case 'Alpha': return 'Tidak hadir tanpa keterangan';
      default: return '';
    }
  }
                            ...
                            GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Icon(Icons.calendar_month, color: AppColor.backgroundColor, size: 18),
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildAturanGaji(),
                            const SizedBox(height: 20),
                            Text('Riwayat Kehadiran Harian', style: PoppinsTextStyle.bold.copyWith(fontSize: 16, color: Colors.black87)),
                            const SizedBox(height: 12),
                            if (riwayatHarian.isEmpty)
                              _buildEmptyHistory()
                            else
                              ...riwayatHarian.map((data) => _buildRiwayatCard(data)).toList(),
                            const SizedBox(height: 20),
                          ],
                        ),
                      ),
        ...
        children: [
          Row(
            children: [
              ...
              Text('Pendapatan', style: PoppinsTextStyle.semiBold.copyWith(fontSize: 14, color: Colors.green[900])),
            ],
          ),
          const SizedBox(height: 16),
          _buildDetailRow('Gaji Pokok', totalGajiPokok, Colors.green[700]!),
          _buildDetailRow('Tunjangan Makan & Transport', totalTunjangan, Colors.green[700]!),
          const Divider(height: 20),
          _buildDetailRow('Total Pendapatan', totalGajiPokok + totalTunjangan, Colors.green[900]!, isBold: true),
        ],
      ),
    );
  }
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 12),
              Text('Potongan', style: PoppinsTextStyle.semiBold.copyWith(fontSize: 14, color: Colors.red[900])),
            ],
          ),
          const SizedBox(height: 16),
          _buildDetailRow('Potongan Keterlambatan', totalPotongan, Colors.red[700]!),
          _buildDetailRow('Pajak Bulanan', pajakBulanan, Colors.red[700]!),
          const Divider(height: 20),
          _buildDetailRow('Total Potongan', totalPotongan + pajakBulanan, Colors.red[900]!, isBold: true),
        ],
      ),
    );
  }
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text('Keterangan Aturan Gaji', style: PoppinsTextStyle.semiBold.copyWith(fontSize: 13, color: Colors.blue[900])),
            ],
          ),
          const SizedBox(height: 12),
          _buildKeteranganItem('• Jam kerja: 07:30 - 17:00'),
          _buildKeteranganItem('• Gaji pokok Rp 50.000/hari (absen masuk & keluar tepat waktu)'),
          _buildKeteranganItem('• Tunjangan Rp 25.000/hari (jika hadir)'),
          _buildKeteranganItem('• Potongan Rp 25.000 jika terlambat masuk'),
          _buildKeteranganItem('• Potongan Rp 25.000 jika pulang lebih awal'),
          _buildKeteranganItem('• Cuti disetujui: dapat gaji pokok, tidak dapat tunjangan'),
          _buildKeteranganItem('• Izin disetujui: tidak dapat gaji pokok dan tunjangan'),
          _buildKeteranganItem('• Alpha/Pending: tidak dapat gaji pokok dan tunjangan'),
          _buildKeteranganItem('• Pajak bulanan: Rp 100.000'),
        ],
      ),
    );
  }
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 16),
            Text('Riwayat Kehadiran Kosong', style: PoppinsTextStyle.semiBold.copyWith(fontSize: 16, color: Colors.grey[700])),
            SizedBox(height: 8),
            Text('Belum ada data kehadiran untuk\n$selectedMonth', textAlign: TextAlign.center, style: PoppinsTextStyle.regular.copyWith(fontSize: 14, color: Colors.grey[600])),
          ],
        ),
      ),
    );
  ...
  Widget _buildDetailRow(String label, int amount, Color color, {bool isBold = false}) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(label, style: (isBold ? PoppinsTextStyle.semiBold : PoppinsTextStyle.regular).copyWith(fontSize: 12, color: color)),
          Text('Rp ${NumberFormat('#,###', 'id_ID').format(amount)}', style: (isBold ? PoppinsTextStyle.semiBold : PoppinsTextStyle.regular).copyWith(fontSize: 12, color: color)),
        ],
      ),
    );
  }
  ...
  Widget _buildKeteranganItem(String text) {
    return Padding(
      ...
      child: Text(text, style: PoppinsTextStyle.regular.copyWith(fontSize: 11, color: Colors.blue[900], height: 1.5)),
    );
  }
  ...
  Widget _buildRiwayatCard(Map<String, dynamic> data) {
    ...
    final totalHari = (data['gajiPokok'] as int) + (data['tunjangan'] as int) - (data['potongan'] as int);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(data['tanggal'], style: PoppinsTextStyle.semiBold.copyWith(fontSize: 13, color: Colors.black87)),
                ],
              ),
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text(data['status'], style: PoppinsTextStyle.semiBold.copyWith(fontSize: 11, color: statusColor)),
                  ],
                ),
              ),
            ],
          ...
          Row(
            children: [
              Expanded(child: Row(children: [Icon(Icons.login, size: 14, color: Colors.grey[600]), const SizedBox(width: 6), Text('Masuk: ${data['jamMasuk']}', style: PoppinsTextStyle.regular.copyWith(fontSize: 11, color: Colors.grey[700]))])),
              Expanded(child: Row(children: [Icon(Icons.logout, size: 14, color: Colors.grey[600]), const SizedBox(width: 6), Text('Keluar: ${data['jamKeluar']}', style: PoppinsTextStyle.regular.copyWith(fontSize: 11, color: Colors.grey[700]))])),
            ],
          ),
          const SizedBox(height: 12),
          Container(
            ...
            child: Column(
              children: [
                _buildRiwayatDetailRow('Gaji Pokok', data['gajiPokok'], Colors.green[700]!),
                const SizedBox(height: 4),
                _buildRiwayatDetailRow('Tunjangan', data['tunjangan'], Colors.green[700]!),
                if ((data['potongan'] as int) > 0) ...[
                  const SizedBox(height: 4),
                  _buildRiwayatDetailRow('Potongan', data['potongan'], Colors.red[700]!, isMinus: true),
                ],
                const Divider(height: 16),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Total Hari Ini', style: PoppinsTextStyle.semiBold.copyWith(fontSize: 12, color: Colors.black87)),
                    Text('Rp ${NumberFormat('#,###', 'id_ID').format(totalHari)}', style: PoppinsTextStyle.bold.copyWith(fontSize: 12, color: totalHari > 0 ? Colors.green[700] : Colors.grey[700])),
                  ],
                ),
              ],
            ),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 6),
                Expanded(child: Text(_getKeterangan(data), style: PoppinsTextStyle.regular.copyWith(fontSize: 10, color: statusColor.withOpacity(0.9)))),
              ],
            ),
          ),
        ],
  ...
  Widget _buildRiwayatDetailRow(String label, int amount, Color color, {bool isMinus = false}) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: PoppinsTextStyle.regular.copyWith(fontSize: 11, color: Colors.grey[700])),
        Text('${isMinus ? "- " : ""}Rp ${NumberFormat('#,###', 'id_ID').format(amount)}', style: PoppinsTextStyle.medium.copyWith(fontSize: 11, color: color)),
      ],
    );
  }
}
...
class _PDFViewerPageState extends State<PDFViewerPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.title, style: PoppinsTextStyle.bold.copyWith(fontSize: 16)),
        backgroundColor: AppColor.primaryColor,
        actions: [
          IconButton(
